...
setupPredict <- function(trait=traits[1], pwd=pwd, mysh="prediction_s2.sh"){
  for(j in 1:1){ ### change j to add more randomization 
    ...
    for(k in 1:5){
      ...
      mymrkRes <- paste("/home/NSF-SAM-GS/HGWAS/SNPdiallel/dcv1/", trait,
    ...
    for(k in 1:5){
      ...
      #map="/home/NSF-SAM-GS/snpmatrix/SAM_SNPs_chrall_samid.map"
      mymrkRes2 <- paste("/home/NSF-SAM-GS/HGWAS/SNPdiallel/dcv1/", trait,
...
pwd = "/home/NSF-SAM-GS/HGWAS/SNPdiallel/predict/s1_predict"

### real runs:
traits <- c("krn", "cd", "cl", "cw", "akw", "tkw", "kc")
setupPredict(trait=traits[1], pwd=pwd)
setupPredict(trait=traits[2], pwd=pwd)
setupPredict(trait=traits[3], pwd=pwd)
setupPredict(trait=traits[4], pwd=pwd)
setupPredict(trait=traits[5], pwd=pwd)
setupPredict(trait=traits[6], pwd=pwd)
setupPredict(trait=traits[7], pwd=pwd)
