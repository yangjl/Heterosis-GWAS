        ...
        for(ti in 1:7){ # traits
            for(j in 1:20){ ### change j to add more randomization 
                ...
                for(k in 1:5){
                    ...
                    inpfile <- paste0("largedata/scripts_cv2/", traits[ti],"_",modei,"_ran", j,"_cv", k, ".inp")
                    
                    mytrain <- paste("largedata/SNPdiallel/dcv2/pheno/", traits[ti],
                                     "_", j, "_train_s", k, ".txt", sep="")
                    mytest <- paste("largedata/SNPdiallel/dcv2/pheno/", traits[ti],
...
files <- list.files(path="largedata/scripts_cv2", pattern="inp", full.names=TRUE)
logs <- gsub("inp", "log", files)
for(i in 1:length(files)){
    
    ### gerpIBD command goes into the sh
    sh1 <- paste("GenSel4R", files[i], ">", logs[i])
    cat(sh1, file=paste0("largedata/scripts_cv2/run_cv_job",i,".sh"), sep="\n", append=FALSE)
    
}

source("~/Documents/Github/zmSNPtools/Rcodes/set_arrayjob.R")
set_arrayjob(shid="largedata/runcv2_array.sh",
             shcode=paste0("sh ", "largedata/scripts_cv2/run_cv_job$SLURM_ARRAY_TASK_ID.sh"),
             arrayjobs= "1-1400",
             wd=NULL, jobid="cv", email="yangjl0930@gmail.com")
